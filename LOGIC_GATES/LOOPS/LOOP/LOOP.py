...
class LoopData:
    ...
    def __init__(self, node_id:str, num_loops:int=-1, current_iteration:int=0, is_finished:bool=False) -> None:
    ...
    def get_data(self):
        return {'node_id': self.node_id, 'num_loops': self.num_loops, 'current_iteration': self.current_iteration, 'is_finished': self.is_finished}

    @staticmethod
    def from_data(node_id:str, data: dict[str,Any]):
        loop_data = LoopData(node_id, num_loops=data.get('num_loops', -1), current_iteration=data.get('current_iteration', 0), is_finished=data.get('is_finished', False))
        return loop_data
...
@flojoy
def LOOP(default: DataContainer, default_parmas: DefaultParams, num_loops: int=-1) -> dict[str,Any]:
...
def build_result(inputs:list[DataContainer], is_loop_finished: bool):
    return JobResultBuilder().from_inputs(inputs).flow_by_flag(flag=is_loop_finished, directionsWhenFalse=['body'], directionsWhenTrue=['end']).build()