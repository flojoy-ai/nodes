...
def test_DF_2_ORDEREDTRIPLE_column_errorMessage(mock_flojoy_decorator):
    import DF_2_ORDEREDTRIPLE
    ...
    np.testing.assert_raises_regex(
        ...
        DF_2_ORDEREDTRIPLE.DF_2_ORDEREDTRIPLE,
        DataFrame(df=df),
    )


def test_DF_2_ORDEREDTRIPLE_columns_switch_and_dtype(mock_flojoy_decorator):
    import DF_2_ORDEREDTRIPLE
    

    df = pd.DataFrame({'hello':[0, 1, 2], '-':[3, 4, 5], 'world':[6, 7, 8]})
    out = DF_2_ORDEREDTRIPLE.DF_2_ORDEREDTRIPLE(DataFrame(df=df), 0, 2, 1)
