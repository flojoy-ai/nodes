...
def obama_segmentation_array_rgb():
    _image_path = f"{os.path.dirname(os.path.realpath(__file__))}/obama_segmentation_image.jpg"
    image = Image.open(_image_path).convert("RGB")
    return np.array(image)


def test_DEEPLAB_V3(mock_flojoy_decorator, obama_image_array_rgb, obama_segmentation_array_rgb):
    import DEEPLAB_V3
