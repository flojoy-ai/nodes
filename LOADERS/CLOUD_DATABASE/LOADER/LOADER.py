...
@flojoy
def LOADER(default: DataContainer, default_parmas: DefaultParams, measurement_uuid: str='') -> DataContainer:
    ...
    if dc_inputs:
        resp = requests.post(MEASUREMENT_API, json={'api_key': api_key, 'measurement_id': measurement_uuid, 'measurement': json.dumps(dc_inputs[0], cls=PlotlyJSONEncoder)})
        if not (resp.status_code == 200 or resp.status_code == 201):
            raise Exception(str(resp.json()['error']))
        return dc_inputs[0]
    else:
        resp = requests.get(MEASUREMENT_API, params={'api_key': api_key, 'measurement_id': measurement_uuid})
        print(resp.json())
        if not (resp.status_code == 200 or resp.status_code == 201):
            raise Exception(resp.json()['error'])
        return DataContainer(x=resp.json()['data'][0]['dataContainer']['x'], y=resp.json()['data'][0]['dataContainer']['y'])