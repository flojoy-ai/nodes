...
@flojoy
def LOCAL_FILE(default: DataContainer, default_parmas: DefaultParams, file_type: Literal['image', 'csv', 'json', 'xml', 'excel']='image', path: str='') -> Union[DataContainer, dict]:
    ...
    match file_type:
        case 'image':
            default_image_path = path.join(path.dirname(path.abspath(__file__)), 'assets', 'astronaut.png')
        ...
        case 'excel':
            ...
            return DataContainer(type='dataframe', m=df)
        case _:
            raise ValueError(f"LOCAL_FILE currently doesn't support file type : {file_type}")