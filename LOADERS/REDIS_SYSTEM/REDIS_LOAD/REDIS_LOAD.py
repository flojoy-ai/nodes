from flojoy import flojoy, JobResultBuilder, OrderedPair, NodeReference, DataContainer
import os
...
@flojoy
def REDIS_LOAD(default: DataContainer, referred_node: NodeReference) -> OrderedPair:
    ...
    if referred_node != "":
        ...
        try:
            ...
            y = x if len(default) > 0 else [1, 3, 2]
            print(traceback.format_exc())
            pass
        return (
            JobResultBuilder()
            .from_inputs([OrderedPair(x=np.arange(0, len(y), 1), y=y)])
            .build()
        )
    else:
