...
def match_figure(
    ...
    dc: OrderedPair | Matrix | DataFrame,
):
    match figure_type:
        case "bar":
...
def add_bar_trace(fig: go.Figure, dc: OrderedPair | Matrix | DataFrame):
    if isinstance(dc, DataFrame):
        df = dc.m
        first_col = df.iloc[:, 0]
        is_timeseries = False
        if is_timeseries:
    ...
    elif isinstance(dc, OrderedPair):
        x = dc.x
...
def add_histogram_trace(fig: go.Figure, dc: OrderedPair | Matrix | DataFrame):
    if isinstance(dc, DataFrame):
        df = dc.m
        for col in df.columns:
...
def add_line_trace(fig: go.Figure, dc: OrderedPair | Matrix | DataFrame):
    if isinstance(dc, DataFrame):
        df = dc.m
        first_col = df.iloc[:, 0]
        is_timeseries = False
...
def add_scatter_trace(fig: go.Figure, dc: OrderedPair | Matrix | DataFrame):
    if isinstance(dc, OrderedPair):
        x = dc.x
        if isinstance(dc.x, dict):
            dict_keys = list(dc.x.keys())
            x = dc.x[dict_keys[0]]
